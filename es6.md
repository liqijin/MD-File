本文整理出来的笔记都是书中的精华内容，囊括了整个`ES6体系`的所有特性，非常方便大家重新认识`全部ES6特性`。半小时的阅读就可以对`ES6`有一个全面的了解，可以认为是一本`ES6特性小字典`，收藏后可随时查阅。

![img](https://mmbiz.qpic.cn/mmbiz_png/v735W8ZTLWnQOiaiakheBp4LD1r40VoOu04fZf9ZWuQJdPdsAQLJUib1icpVSRRgWLmSwDz5dib2uSzgahX3K0gBr0A/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)ES6缩略
...
```
// 常量跨文件共享// person.jsconst NAME = "Bruce";const AGE = 25;const SEX = "male";export { AGE, NAME, SEX };
// file1.jsimport { AGE } from "person";console.log(AGE);
// file2.jsimport { AGE, NAME, SEX } from "person";console.log(AGE, NAME, SEX);
```

> 默认导入互换整体导入

```
import Person from "person";console.log(Person.AGE);
import * as Person from "person";console.log(Person.default.AGE);
```
...
```
function Wrapper(func) {    return function(...args) {        const generator = func(...args);        generator.next();        return generator;    }}const print = Wrapper(function*() {    console.log(`First Input: ${yield}`);    return "done";});print().next("hello");
```
...
# ES5 到 ES2026：JavaScript 完整进化史（核心特性全解）

本文按**年份版本**系统梳理 ECMAScript 从 2009 年 ES5 到 2026 年的所有核心新特性，覆盖语法、数据结构、异步、类与模块等关键维度，附代码示例与使用场景，适合面试复习与开发参考。

------

## 一、ES5（ECMAScript 5，2009）：基础强化

作为现代 JS 的基石，ES5 引入了严格模式与核心 API 增强：

|       特性        |                         说明                          |                代码示例                 |
| :---------------: | :---------------------------------------------------: | :-------------------------------------: |
|   **严格模式**    |   禁止隐式全局变量、`this`指向全局、重复参数等错误    |             `"use strict";`             |
| **getter/setter** |           对象属性访问器，实现属性读写控制            | `const obj = { get x() { return 1; } }` |
|  **Array 扩展**   | `Array.isArray()`、`forEach()`、`map()`、`filter()`等 |         `[1,2,3].map(x => x*2)`         |
|  **Object 扩展**  |     `Object.defineProperty()`、`Object.freeze()`      |         `Object.freeze({a:1})`          |
|   **JSON 支持**   |          `JSON.parse()`、`JSON.stringify()`           |         `JSON.parse('{"a":1}')`         |

------

## 二、ES6（ES2015）：革命性更新

JS 里程碑版本，引入**类、模块、箭头函数**等核心特性，彻底改变开发方式：

### 1. 变量与作用域

- let/const：块级作用域，解决var变量提升与重复声明问题
...
- 箭头函数：简洁语法，无自身this，继承外层作用域

  ```javascript
  const add = (a,b) => a + b;
  ```

- **参数默认值**：`function f(x=1) { ... }`

- **剩余参数**：`function f(...args) { ... }`（替代`arguments`）
...
- class 语法糖：简化原型继承，支持constructor、静态方法
...
- import/export：原生模块系统，替代 CommonJS/AMD
...
- **Promise**：异步编程基础，解决回调地狱
- **Symbol**：唯一值类型，用于对象私有属性
- **for...of 循环**：遍历可迭代对象（数组、Set 等）
...
1. 数组 includes ()：替代indexOf()，更直观判断元素存在
...
## 四、ES2017（ES8）：异步与对象增强

### 1. **async/await**：异步编程革命

基于 Promise 的同步写法，彻底解决回调地狱
...
- Object.values()/entries()：获取对象值 / 键值对数组
...
- **函数参数尾逗号**：允许函数定义 / 调用时参数末尾逗号

------

## 五、ES2018（ES9）：异步迭代与正则增强
...
- **后行断言**：`(?<=...)`（正向后行）、`(?（负向后行）
- **Unicode 属性转义**：`\p{...}`匹配 Unicode 字符属性
- **命名捕获组**：`/(?\d{4})/`，结果可通过名称访问

### 3. 其他

- **对象扩展运算符**：`const newObj = {...obj, c:3}`
- **Promise.finally()**：无论 Promise 成功失败都执行

------

## 六、ES2019（ES10）：细节打磨与 API 完善
...
2. **String.trimStart()/trimEnd()**：替代`trimLeft()`/`trimRight()`
...
4. **Symbol.description**：获取 Symbol 描述

5. **可选 catch 绑定**：`try { ... } catch { ... }`（无需声明 error 参数）

------

## 七、ES2020（ES11）：现代语法补全

### 1. 核心语法

- 空值合并运算符：??，仅当左侧为null/undefined时返回右侧
...
- 可选链操作符：?.，安全访问嵌套对象属性
  ...
  ```javascript
  const name = user?.profile?.name; // 避免Cannot read property错误
  ```

### 2. 其他关键

- BigInt：任意精度整数，解决 Number 精度限制

  ```javascript
  const big = 100n; // 后缀n表示BigInt
  ```

- **globalThis**：统一全局对象访问（浏览器`window`、Node.js`global`）

- **Promise.allSettled()**：等待所有 Promise 完成（无论成功失败）

- **动态 import ()**：异步加载模块，返回 Promise
...
3. **String.replaceAll()**：替换所有匹配项，无需正则`/g`修饰符

4. **Promise.any()**：返回第一个成功的 Promise，拒绝所有则抛出 AggregateError

------

## 九、ES2022（ES13）：类的蜕变与私有成员
...
- 私有字段 / 方法：#前缀定义，外部无法访问
  ...
  class Person {
    #name; // 私有字段
    constructor(name) { this.#name = name; }
    #say() { console.log(this.#name); } // 私有方法
  }
  ```

- 静态字段 / 静态块：类级别的静态属性与初始化块
...
- **类字段声明**：无需在 constructor 中定义实例字段

### 2. 其他

- Top-level await：模块顶层直接使用 await，无需 async 函数
...
## 十、ES2023（ES14）：不可变操作与数组增强

### 1. 数组不可变方法

新增返回**新数组**的方法，避免原数组修改：

```javascript
const arr = [1,2,3];
arr.toSorted(); // [1,2,3] 原数组不变
arr.toReversed(); // [3,2,1]
arr.with(1, 5); // [1,5,3] 替换索引1的值
```

### 2. 数组查找增强

- findLast()/findLastIndex()：从数组末尾开始查找
...
- 允许 Symbol 作为 WeakMap 的键，增强灵活性

------

## 十一、ES2024（ES15）：并发与 Unicode

### 1. 并发原语

- **Atomics.waitAsync()**：异步等待共享内存变化，避免阻塞主线程
- **Float16Array**：半精度浮点数数组，适合 WebGL 等场景
...
- Set.prototype.intersection()/union()/difference()：集合操作
...
- **RegExp v 标志**：Unicode 属性转义增强
- **String.prototype.isWellFormed()/toWellFormed()**：处理 Unicode 非法序列

------

## 十二、ES2025（ES16）：迭代器与模块增强

### 1. 迭代器 API

- Iterator 全局对象：提供迭代器工具方法

  ```javascript
  Iterator.from([1,2,3]); // 生成迭代器
  Iterator.range(1,5); // 生成1-5的迭代器
  ```

### 2. 模块与 JSON

- Import Attributes：明确模块类型，支持 JSON 导入
...
## 十三、ES2026（ES17）：草案特性（预计）

目前处于 TC39 提案阶段，预计纳入以下核心特性：

1. **管道运算符**：`|>`，函数链式调用更直观

   ```javascript
   const result = 5 |> double |> square; // 等价于square(double(5))
   ```

2. **记录与元组**：`record`（不可变对象）、`tuple`（不可变数组）

   ```javascript
   const rec = #{ x: 1, y: 2 }; // 不可变记录
   const tup = #[1,2,3]; // 不可变元组
   ```

3. **装饰器**：类、方法、属性装饰，增强元编程能力
...
4. **模式匹配**：类似 switch 的高级条件匹配

   ```javascript
   match (value) {
     when 1: console.log('One');
     when 2: console.log('Two');
...
|     阶段     |   版本    |      核心方向      |             标志性特性              |
| :----------: | :-------: | :----------------: | :---------------------------------: |
|   基础巩固   |    ES5    | 严格模式、API 完善 |     严格模式、Array 方法、JSON      |
|   语法革命   |    ES6    |    现代语言特性    |   let/const、class、Promise、模块   |
|   异步完善   |  ES7-ES9  | 异步编程、正则增强 | async/await、Promise 扩展、异步迭代 |
|   细节打磨   | ES10-ES12 |  语法糖、API 补全  |     空值合并、可选链、逻辑赋值      |
|   类与模块   |   ES13    |    面向对象增强    |    私有成员、静态块、顶层 await     |
| 不可变与集合 | ES14-ES15 |    数据操作优化    |  数组不可变方法、集合运算、groupBy  |
|   未来展望   |  ES2026+  |   元编程、函数式   |    管道运算符、记录元组、装饰器     |

------

### 学习建议

1. **掌握 ES6 核心**：let/const、箭头函数、解构、Promise、class 是现代 JS 基础
2. **重点关注 ES2020+**：可选链、空值合并、私有成员、顶层 await 是高频开发特性
3. **了解 ES2024 + 新特性**：集合运算、groupBy、Promise.try 等提升开发效率
4. **关注 ES2026 草案**：管道运算符、装饰器等将改变代码编写方式